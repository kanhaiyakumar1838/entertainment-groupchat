...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    // Join the group room on socket server (guard)
  ...
  // ensure clicking the label triggers file input in all browsers
    ...
    >
          ...
          <div key={msg._id || msg.id || index} style={{ marginBottom: 10 }}>
            <div style={{ fontSize: 13, color: "#333" }}>
              ...
              <span style={{ marginLeft: 8, color: "#888", fontSize: 12 }}>
                {new Date(msg.createdAt || msg.createdAtAt || Date.now()).toLocaleString()}
              </span>
            </div>
      ...
      >
        <div style={{ position: "relative" }}>
          <button onClick={() => setShowMenu((s) => !s)} style={{ padding: 6, borderRadius: 8 }}>
            <FaPlus />
          </button>
          ...
          {showMenu && (
            ...
            >
              <div style={{ display: "flex", gap: 6 }}>
                <div style={menuItemStyle} onClick={handleUploadClick}>
      ...
      {floatingVideo && (
        <FloatingYouTube videoId={floatingVideo.videoId} title={floatingVideo.title} initial={{ x: 80, y: 80, width: 560 }} onClose={() => setFloatingVideo(null)} />
      )}
    </div>
  );
}
